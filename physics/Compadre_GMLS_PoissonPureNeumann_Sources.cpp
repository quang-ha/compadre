...
void GMLS_PoissonPureNeumannSources::evaluateRHS(local_index_type field_one, local_index_type field_two, scalar_type time) {
    ...
    // obtaint the ID for the solution field
    ...
    for (local_index_type i=0; i<nlocal; i++) {
        for (local_index_type k=0; k < fields[field_one]->nDim(); k++) {
            ...
            if (field_one == solution_field_id && field_two == solution_field_id) {
                rhs_vals(dof, 0) = function->evalScalarLaplacian(pt);
            }
        }
    }
    ...
    if (field_one == solution_field_id && field_two == solution_field_id) {
        ...
        for (local_index_type i=0; i<num_boundary; i++) {
            for (local_index_type k=0; k<fields[field_one]->nDim(); k++) {
                ...
                scalar_type b_i = _physics->_solution_neumann_GMLS->getAlpha0TensorTo0Tensor(TargetOperation::DivergenceOfVectorPointEvaluation, i, num_neighbors);
                ...
                xyz_type force_term = function->evalScalarDerivative(pt);
                scalar_type g = force_term.x*_physics->_solution_neumann_GMLS->getTangentBundle(i, 2, 0)
                    + force_term.y*_physics->_solution_neumann_GMLS->getTangentBundle(i, 2, 1)
                    + force_term.z*_physics->_solution_neumann_GMLS->getTangentBundle(i, 2, 2);
...
std::vector<InteractingFields> GMLS_PoissonPureNeumannSources::gatherFieldInteractions() {
